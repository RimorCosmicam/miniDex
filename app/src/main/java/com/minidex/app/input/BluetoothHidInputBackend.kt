...
class BluetoothHidInputBackend(private val context: Context) : InputBackend {
    ...
    override val name: String = "Native Bluetooth HID (Hardware Emulation)"
    override val requiresPrivilegedAccess: Boolean = false

    override val isAvailable: Boolean
        get() = isHidDeviceSupported()
    ...
    private val callback = object : BluetoothHidDevice.Callback() {
        override fun onConnectionStateChanged(device: BluetoothDevice?, state: Int) {
            ...
            if (state == BluetoothProfile.STATE_CONNECTED) {
                ...
                Log.i(TAG, "Bluetooth HID Connected to: ${device?.name ?: device?.address}")
    ...
    override suspend fun initialize(): Result<Unit> {
        val adapter = BluetoothAdapter.getDefaultAdapter()
            ?: return Result.failure(IllegalStateException("Bluetooth not available on this device"))
